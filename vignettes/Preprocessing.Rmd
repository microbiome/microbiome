...
Z transforming the counts

```{r preprocessing-ztrans}
# Z transform samples
pseq.z <- transform_sample_counts(pseq, function(x) { y <- log10(1 + x); (y - mean(y))/sd(y) })

# Z transform OTUs
pseq.z <- pseq
trans <- as.matrix(t(scale(t(log10(1 + otu_table(pseq)@.Data)))))
otu_table(pseq.z) <- otu_table(trans, taxa_are_rows=taxa_are_rows(physeq))
```
