...
```{r matvisu-example, fig.width=8, fig.height=16}
# Load example data
library(microbiome)
data(peerj32)
x <- peerj32$microbes
...
```{r heatmap-crosscorrelate2, message=FALSE, warning=FALSE, fig.keep='none'}
hm <- heatmap(x) 
```

Then plot the same matrix with ordered rows and cols:

```{r heatmap-crosscorrelate3, message=FALSE, warning=FALSE}
