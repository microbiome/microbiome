 ...
 * ```r round(100 * prop.table(table(subset(sample_data(atlas), gender %in% c("male", "female"))$gender))[["female"]], 1)```% female ratio. Altogether, gender information is available for ```round(100 * sum(table(subset(sample_data(atlas))$gender)[c("male", "female")])/nsamples(atlas), 1)```% of the samples.
...
for (varname in c("sample_type", "DNA_extraction_method", "nationality", "age", "bmi", "bmi_group")) {
...
```{r hatlas-example3, fig.width=8, fig.height=5, message=FALSE, warning=FALSE, show="hold", out.width="400px"}
