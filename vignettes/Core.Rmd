...
Pick the core microbiota subset of the data. This corresponds to taxa that exceed the given prevalence and detection thresholds.

```{r core-data, message=FALSE, warning=FALSE}
pseq.core <- core(pseq.rel, detection = 1, prevalence = 50)
```

Alternatively, use bootstrap analysis to identify the core members (for added robustness; see [Salonen et al. CMI (2012)](http://onlinelibrary.wiley.com/doi/10.1111/j.1469-0691.2012.03855.x/abstract). The core members may be sensitive to sampling effects; bootstrap aims to estimate how frequently a particular group is determined to be a core member in random bootstrap subsets of the data.

```{r core_bootstrap, message=FALSE, warning=FALSE}
