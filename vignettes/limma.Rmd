...
```{r limma-qq, warning=FALSE, fig.width=8, fig.height=8, fig.show="hold"}
# QQ
...
A standard method for linear modeling of high-throughput bioinformatics data is [limma](http://bioinf.wehi.edu.au/limma/). See also
[limma User's guide](http://www.lcg.unam.mx/~lcollado/R/resources/limma-usersguide.pdf). For discussion on why limma is preferred over t-test, see [this
article](http://www.plosone.org/article/info:doi/10.1371/journal.pone.0012336).
...
into account in this wrapper. See the [limma examples](limma.md) for
more info.

```{r limma-lm-phyloseq, warning=FALSE}
library(limma)
library(microbiome)
data(atlas1006)
...
```{r limma-example2222, warning=FALSE}
# Get example data
library(microbiome)
data("peerj32")
