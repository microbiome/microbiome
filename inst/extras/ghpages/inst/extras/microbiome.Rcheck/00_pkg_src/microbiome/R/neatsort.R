...
neatsort <- function (x, target, method = "NMDS", distance = "bray",
  ...
  if (is.phyloseq(x)) {
    ...
    if (!method == "NMDS") {
      warning("Only NMDS dissimilarity implemented for matrices. Using NMDS for sorting.")
      method <- "NMDS"
    }
  ...
  # Quick fix: the scores function fails otherwise.
  disp.target <- target
  if (target == "species" && !is.phyloseq(xo)) {disp.target <- "sites"}

  tmp <- try({DF <- scores(ord, choices = c(1, 2), display = disp.target)}, silent = TRUE)
