...
#'    species and has been used in microbiomics context for instance in
...
#' @keywords utilities
dominance <- function(x, index = "all", rank = 1,
                      relative = TRUE, aggregate = TRUE) {
    
    # Only include accepted indices
    accepted <- c("DBP", "DMN", "absolute", "relative",
    ...
    if (length(index) > 1) {
        ...
        for (idx in index) {
            tab <- cbind(tab, dominance(x, index = idx, rank = rank,
	             relative = relative, aggregate = aggregate))
        }
...
simpson_dominance <- function(x, zeroes = TRUE) {
    ...
    # More advanced Simpson dominance (Simpson 1949) However let us not
    # use this as it is not in [0,1] and it is very highly correlated with
    # the simpler variant lambda Species richness (number of species)
    # S <- length(x) sum(p * (p - 1)) /
    # (S * (S - 1))
