...
#' @param threshold Indicates the fraction  of the ecosystem to be occupied by the N most abundant species (N is returned by this function). If the detection argument is a vector, then a data.frame is returned, one column for each detection threshold.
...
coverage <- function(x, threshold = 0.5) {
  ...
  if (length(threshold) > 1) {
    tab <- sapply(threshold, function (th) {coverage(x, threshold = th)})
