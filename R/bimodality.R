...
multimodality <- function(x, peak.threshold = 1, bw.adjust = 1, bs.iter = 100, min.density = 1, 
    ...
    if (is.vector(x)) {
        ...
        # Add small noise to enable robust density estimation (identical values may cause
        # failure)
        x <- x + rnorm(length(x), sd = sd(x)/100)
        m <- potential_analysis(x, bw.adjust = bw.adjust, bs.iter = bs.iter, min.density = min.density)
        ...
        for (tax in rownames(x)) {
            ...
            m <- multimodality(as.numeric(x[tax, ]), peak.threshold, bw.adjust, bs.iter, 
                min.density, verbose)
            nmodes[[tax]] <- m$modes
