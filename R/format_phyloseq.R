...
format_phyloseq <- function(x) {
    if (ncol(tax_table(x)) == 6) {
        pobj <- x
        colnames(tax_table(pobj)) <- c("Domain", "Phylum", "Class", "Order", 
            "Family", "Genus")
        ...
        tax <- mutate(x, Domain, Domain = ifelse(Domain == "", "Unclassified", 
            Domain)) %>% mutate(Phylum, Phylum = ifelse(Phylum == "", paste("k__", 
            Domain, "_", rownames(x), sep = ""), Phylum)) %>% mutate(Class, 
            Class = ifelse(Class == "", ifelse(grepl("__", Phylum), Phylum, 
                paste("c__", Phylum, "_", rownames(x), sep = "")), Class)) %>% 
            mutate(Order, Order = ifelse(Order == "", ifelse(grepl("__", 
                Class), Class, paste("c__", Class, "_", rownames(x), sep = "")), 
                Order)) %>% mutate(Family, Family = ifelse(Family == "", 
            ifelse(grepl("__", Order), Order, paste("o__", Order, "_", 
                rownames(x), sep = "")), Family)) %>% mutate(Genus, Genus = ifelse(Genus == 
            "", ifelse(grepl("__", Family), Family, paste("f__", Family, 
            "_", rownames(x), sep = "")), Genus))
        ...
        pobj2 <- x
        colnames(tax_table(pobj2)) <- c("Domain", "Phylum", "Class", "Order", 
        ...
        tax <- mutate(x, Domain, Domain = ifelse(Domain == "", "Unclassified", 
            ...
            ifelse(grepl("__", Order), Order, paste("o__", Order, "_", 
                rownames(x), sep = "")), Family)) %>% mutate(Genus, Genus = ifelse(Genus == 
            "", ifelse(grepl("__", Family), Family, paste("f__", Family, 
            "_", rownames(x), sep = "")), Genus)) %>% mutate(Species, Species = ifelse(Species == 
            "", ifelse(grepl("__", Genus), Genus, paste("f__", Genus, "_", 
            rownames(x), sep = "")), Species))
