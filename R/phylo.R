...
GetPhylogeny <- function (chip, phylogeny.version = "full") {
  ...
  if (chip == "HITChip") {
    ...
    phylogeny.info <- read.profiling(level = paste("phylogeny.", 
                                 phylogeny.version, sep = ""), 
...
levelmap <- function (phylotypes = NULL, level.from, level.to, phylogeny.info) {
  ...
  if (level.from == "L1" && level.to == "L2") {
    ...
    for (pt in phylotypes) {
      sl[[pt]] <- as.character(unique(
            phylogeny.info[phylogeny.info[["L1"]] == pt, "L2"]))
    }
  }     
  ...
  if (level.from == "L1" && level.to == "L0") {
    ...
    for (pt in phylotypes) {
      sl[[pt]] <- as.character(unique(
            phylogeny.info[phylogeny.info[["L1"]] == pt, "L0"]))
    }
  }     
  ...
  if (level.from == "L0" && level.to %in% c("L1", "L2")) {
    ...
    for (pt in phylotypes) {
      sl[[pt]] <- as.character(unique(
            phylogeny.info[phylogeny.info[[level.from]] == pt, 
                                    level.to]))
    }
  }     
...
retrieve.probesets <- function (phylogeny.info, level = "species", 
                                     name = NULL) {
