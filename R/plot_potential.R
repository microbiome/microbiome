...
#' @keywords utils
plot_potential <- function(res, cutoff = 0.5, plot.contours = TRUE, binwidth = 0.2, 
    bins = NULL) {
    
    
    # TODO Experimental implementation including bootstrap as in Lahti et al (2014)
    # Nat Comm potential_analysis(diversity, age) to replace movpotential_ews, see
    # potential_analysis.R does not work with visualization since not meant for that
    # purpose TODO enable plotting from potential_analysis omitting bootstrap to get
    # rid of earlywarnings dependency
    ...
    cut.potential <- max(apply(potentials, 1, min)) + cutoff * abs(max(apply(potentials, 
    ...
    intp <- loess_interpolation(as.vector(covariates), as.vector(grid), as.vector(potentials), 
    ...
    if (plot.contours) {
        if (!is.null(bins)) {
            ...
            p <- p + stat_contour(bins = bins, aes(x = bg.var, y = phylotype, z = potential, 
                fill = potential))
        } else {
            p <- p + stat_contour(binwidth = binwidth, aes(x = bg.var, y = phylotype, 
...
loess_interpolation <- function(x, y, z, gridn = c(40, 40), span = 0.1, ...) {
    ...
    if (length(x) < 30 && span < 0.5) {
        warning("with less than 30 points, use span >> 0.5 or use akima", immediate. = TRUE)
        message(paste("trying span ", span, "for", length(x), "points\n"))
    }
    x0 <- seq(min(x), max(x), length = gridn[[1]])
    y0 <- seq(min(y), max(y), length = gridn[[2]])
    
    coords <- suppressWarnings(loess(z ~ x + y, data.frame(x = x, y = y), span = span, 
        ...))
