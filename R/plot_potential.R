...
plot_potential <- function(res, cutoff = 0.5, plot.contours = TRUE,
    ...
    # TODO Experimental implementation including bootstrap as in
    # Lahti et al (2014) Nat Comm
    # potential_analysis(diversity, age) to replace movpotential_ews, see
    # potential_analysis.R does not work with visualization since not meant
    # for that
    # purpose TODO enable plotting from potential_analysis omitting bootstrap
    # to get
    ...
    intp <- loess_interpolation(as.vector(covariates), as.vector(grid),
    ...
    if (plot.contours) {
        if (!is.null(bins)) {
            ...
            p <- p + stat_contour(bins = bins, aes(x = bg.var,
	        y = phylotype, z = potential, 
                fill = potential))
        } else {
