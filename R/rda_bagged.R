...
rda_bagged <- function(x, y, bs.iter = 100, verbose = T) {
    ...
    while (stop.run == FALSE) {
	   ...
	   unlist(sapply(class.split, function(x) sample(x, 
            length(x), replace = T))), simplify = FALSE)
        Bag.res = Bagged.RDA(x, y, bs.iter = boot)
        min.prob = Bag.res$significance[[1]]
        if (length(levels(y)) > 2) {
            for (i in 1:nrow(x)) min.prob[i] = min(sapply(Bag.res$significance, function(x) x[i]))
        }
        ...
        if (nrow(x) > max(length(class.split), 2)) 
            x = x[-which.max(min.prob), ] else stop.run = T
    }
...
Bagged.RDA <- function(X, Y, bs.iter = 100) {
    ...
    if (is.numeric(boot)) {
	    ...
	    unlist(sapply(class.split, function(x) sample(x, length(x), 
            replace = T))), simplify = FALSE)
    }
    bs.iter <- length(boot)
    n.lev <- length(levels(Y))
    TT <- scores(rda(t(X) ~ Y), choices = 1:max(n.lev - 1, 2),
        display = "sites")
    nRDA <- ncol(TT)
    # rotate
    ...
    Tx = lapply(boot, function(x) {
        ...
        err = NA
        if (length(testset) > 0) {
            Pr = predict(M, t(as.data.frame(X[, testset])), type = "wa",
	        model = "CCA")
            centers = apply(TT.m, 2, function(z) sapply(split(z, Y[x]), mean))
            if (nC == 2) 
                y.pred = apply(sapply(Pr, function(x)
		   (x - centers[, 1])^2), 2, which.min)
    ...
    # Group centers
    Group.center = apply(bagged.scores, 2, function(x)
        sapply(split(x, Y), mean))
    err.t = mean((nearest.centers(bagged.scores, Group.center) -
        as.numeric(Y)) != 0)
    
    # bagged error
    err.random = replicate(
        bs.iter, mean((as.numeric(Y) - sample(as.numeric(Y))) != 0))
    ...
    list(loadings = bagged.loadings, scores = bagged.scores,
        ...
        error = bagged.R2, group.centers = Group.center, bootstrapped = Tx,
