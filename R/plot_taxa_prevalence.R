#' @title Plotting the Taxa Prevalence
#' @description A simplified wrapper for creating taxa prevalence plots at
#'    Phylum/Order/Class/Family level.
#' @details This plot is useful to have a first insights into how is the taxa
#'    distribution in your data.
#'    This also gives an idea about the taxonomic affiliation of rare
#'    and abundant taxa in your dataset. This may be helpful in some data
#'    filtering or other downstream analysis.
...
plot_taxa_prevalence <- function(x, level) {
    ...
    if (level == "Phylum") {
        ...
        xdf <- data.frame(abundance = log(tax.abun),
	    prevalence = tax.prev, Phylum)
