...
low_abundance <- function(x, detection = 0.2/100) {
  ...
  if (length(detection) > 1) {
    tab <- sapply(detection, function (th) {low_abundance(x, detection = th)})
