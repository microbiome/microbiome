#' @title Plot RDA
#' @description rda_bagged output visualization.
...
plot_rda_bagged <- function(x, which.bac = 1:nrow(x$loadings),
    ptype = "spider", 
    comp = 1:2, cex.bac = 0.5, plot.names = TRUE,
	...
	cellipse = 0.5, 
            cpoint = 0, add.plot = TRUE)
    
    # Same with ggplot but not ready yet
    skip <- TRUE
    if (!skip) {
        # build ggplot dataframe with points (x,y) and corresponding
	# groups (cluster)
	    ...
	    suffixes = c("", ".centroid"))
        # generate star plot...
        ggplot(gg) + geom_point(aes(x = x, y = y, color = cluster), size = 3) +
    ...
    if (ptype == "hull") {
        ...
        for (i in 1:length(hulls)) polygon(scaled.scores[hulls[[i]], ],
	border = group.cols[i])
    }
    if (plot.names) {
