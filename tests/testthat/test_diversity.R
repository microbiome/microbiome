...
test_that("diversity works correctly", {
  ...
  expect_true(is.vector(diversity(pseq, measures = "Observed", split = FALSE)))
  ...
  expect_true(length(rarity(pseq, detection = 0.1/100, prevalence = 50/100, split = FALSE)) == 1)
 
  expect_true(min(dominance(transform(pseq, "compositional"))) >= 0)
  expect_true(max(top_abundance(transform(pseq, "compositional"))) <= 1)  
